...
concommand.Add( "e2power_pass", function(ply,cmd,argm)
	...
	if !E2Power_pass then printMsg(ply,"the password is disable") return end
...
concommand.Add( "e2power_get_pass", function(ply,cmd,argm)
	if checkPly(ply) then
		if !E2Power_pass then printMsg(ply,"the password is disable") 
		else printMsg(ply,E2Power_pass) end
	end
end )
...
local function E2Power_GetGroupList()
	if !file.Exists( "E2Power/group.txt", "DATA") then 
		E2Power_GroupList={"superadmin","admin"}
		file.Write( "E2Power/group.txt", table.concat(E2Power_GroupList,'\n') ) 
	else
		E2Power_GroupList=string.Explode('\n',file.Read( "E2Power/group.txt", "DATA" ))
		if E2Power_GroupList[1]:len()==0 then E2Power_GroupList={} end
	end
...
local function E2Power_GetBlackList()
...
E2Power_GetBlackList()
E2Power_GetGroupList()
